...
function App() {
  ...
  async function handleCenterClick (centerId) {
    ...
    const { data : planData} = await supabase.from("plan").select().eq("center_id", centerId);
    setPlan(planData);
  }
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-semibold text-center text-blue-600">Swimming Centers</h2>
        ...
        {swimmingCenters.map((center) => (
          ...
          >
            ...
            {selectedCenter === center.center_id && (
              <div className="mt-4">
                <h2 className="text-2xl font-semibold text-center text-blue-600">Plans</h2>
                <div className="flex flex-wrap justify-center">
                  {plan.map((p) => (
                    <div
                      key={p.plan_id}
